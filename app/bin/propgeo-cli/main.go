...
func parseArgs() bool {
	...
	hostname = getEnv("PROPGEO_HOSTNAME", hostname)
	output = getEnv("PROPGEO_OUTPUT", output)
	portStr := getEnv("PROPGEO_PORT", "")
...
func refusedErrorString(addr string) string {
	return fmt.Sprintf("Could not connect to PropGeo at %s: Connection refused", addr)
}
...
func main() {
	...
	password := getEnv("PROPGEO_PASSWORD", "")
...
func help(arg string) error {
	...
	if arg == "" {
		fmt.Fprintf(os.Stderr, "propgeo-cli %s (git:%s)\n", core.Version, core.GitSHA)
