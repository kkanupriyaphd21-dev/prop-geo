...
// for example
//   $ ./propgeo-server --webhook-http-consumer-port 9999
//
...
func main() {
	...
	versionLine := `propgeo-server version: ` + core.Version + gitsha
...
`,
	...
	if log.LogJSON() {
		log.Printf(`PropGeo %s%s %d bit (%s/%s) %s%s, PID: %d. Visit propgeo.com/sponsor to support the project`,
