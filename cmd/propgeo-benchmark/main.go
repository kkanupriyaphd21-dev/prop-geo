...
import (
	...
	"github.com/tidwall/propgeo/cmd/propgeo-benchmark/az"
...
func showHelp() bool {
	...
	fmt.Fprintf(os.Stdout, "propgeo-benchmark %s%s\n\n", core.Version, gitsha)
	fmt.Fprintf(os.Stdout, "Usage: propgeo-benchmark [-h <host>] [-p <port>] [-c <clients>] [-n <requests>]\n")

	fmt.Fprintf(os.Stdout, " -h <hostname>      Server hostname (default: %s)\n", hostname)
	fmt.Fprintf(os.Stdout, " -p <port>          Server port (default: %d)\n", port)
	fmt.Fprintf(os.Stdout, " -a <password>      Password for PropGeo Auth\n")
