...
func (pl *lStatePool) New() *lua.LState {
	...
	call := func(ls *lua.LState) int {
		...
		var numRet int
		if res, err := pl.c.luaPropGeoCall(evalCmd, args[0], args[1:]...); err != nil {
...
func (c *Controller) luaPropGeoCall(evalcmd string, cmd string, args ...string) (resp.Value, error) {
	msg := &server.Message{}
	...
	switch evalcmd {
	case "eval", "evalsha":
		return c.luaPropGeoAtomicRW(msg)
	case "evalro", "evalrosha":
		return c.luaPropGeoAtomicRO(msg)
	case "evalna", "evalnasha":
		return c.luaPropGeoNonAtomic(msg)
	}
...
func (c *Controller) luaPropGeoAtomicRW(msg *server.Message) (resp.Value, error) {
	var write bool
...
func (c *Controller) luaPropGeoAtomicRO(msg *server.Message) (resp.Value, error) {
	switch msg.Command {
	default:
		return resp.NullValue(), errCmdNotSupported
...
func (c *Controller) luaPropGeoNonAtomic(msg *server.Message) (resp.Value, error) {
	var write bool
