...
ACCESS DENIED
PropGeo is running in protected mode because protected mode is enabled, no host
...
want to connect from external computers to PropGeo you may adopt one of the
following solutions:

1) Disable protected mode by sending the command 'CONFIG SET protected-mode no'
   from the loopback interface by connecting to PropGeo from the same host
   the server is running, however MAKE SURE PropGeo is not publicly accessible
   from internet if you do so. Use CONFIG REWRITE to make this change
   permanent.
2) Alternatively you can just disable the protected mode by editing the PropGeo
