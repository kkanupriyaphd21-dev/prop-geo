...
func (pl *lStatePool) New() *lua.LState {
	...
	call := func(ls *lua.LState) int {
		...
		var numRet int
		if res, err := pl.s.luaPropGeoCall(evalCmd, args[0], args[1:]...); err != nil {
...
func (s *Server) luaPropGeoCall(evalcmd string, cmd string, args ...string) (resp.Value, error) {
	msg := &Message{}
	msg.OutputType = RESP
	...
	switch evalcmd {
	case "eval", "evalsha":
		return s.luaPropGeoAtomicRW(msg)
	case "evalro", "evalrosha":
		return s.luaPropGeoAtomicRO(msg)
	case "evalna", "evalnasha":
		return s.luaPropGeoNonAtomic(msg)
	}
...
func (s *Server) luaPropGeoAtomicRW(msg *Message) (resp.Value, error) {
	var write bool
...
func (s *Server) luaPropGeoAtomicRO(msg *Message) (resp.Value, error) {
	switch msg.Command() {
	default:
		return resp.NullValue(), errCmdNotSupported
...
func (s *Server) luaPropGeoNonAtomic(msg *Message) (resp.Value, error) {
	var write bool
