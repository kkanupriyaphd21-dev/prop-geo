...
func (c *Server) extStats(m map[string]interface{}) {
	...
	m["propgeo_aof_rewrite_in_progress"] = c.shrinking
	// Length of time the last AOF shrink took
	m["propgeo_aof_last_rewrite_time_sec"] = c.lastShrinkDuration.get() / int(time.Second)
	...
	m["propgeo_aof_size"] = c.aofsz
	// Whether or no the HTTP transport is being served
	m["propgeo_http_transport"] = c.http
	// Number of connections accepted by the server
	m["propgeo_total_connections_received"] = c.statsTotalConns.get()
	// Number of commands processed by the server
	m["propgeo_total_commands_processed"] = c.statsTotalCommands.get()
	// Number of webhook messages sent by server
	m["propgeo_total_messages_sent"] = c.statsTotalMsgsSent.get()
	// Number of key expiration events
	m["propgeo_expired_keys"] = c.statsExpired.get()
	// Number of connected slaves
	m["propgeo_connected_slaves"] = len(c.aofconnM)
	...
	m["propgeo_num_collections"] = c.cols.Len()
	// Number of hooks in the database
	m["propgeo_num_hooks"] = len(c.hooks)
