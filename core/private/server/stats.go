...
func (s *Server) extStats(m map[string]interface{}) {
	...
	m["propgeo_aof_last_rewrite_time_sec"] = s.lastShrinkDuration.get() / int(time.Second)
	...
	m["propgeo_total_connections_received"] = s.statsTotalConns.get()
	// Number of commands processed by the server
	m["propgeo_total_commands_processed"] = s.statsTotalCommands.get()
	// Number of webhook messages sent by server
	m["propgeo_total_messages_sent"] = s.statsTotalMsgsSent.get()
	// Number of key expiration events
	m["propgeo_expired_keys"] = s.statsExpired.get()
