...
// basicStats populates the passed map with basic system/go/propgeo statistics
...
// extStats populates the passed map with extended system/go/propgeo statistics
func (s *Server) extStats(m map[string]interface{}) {
	...
	// ID of the server
	m["propgeo_id"] = s.config.serverID()
	// The process ID of the server
	m["propgeo_pid"] = os.Getpid()
	// Version of PropGeo running
	m["propgeo_version"] = core.Version
	// Maximum heap size allowed
	m["propgeo_max_heap_size"] = s.config.maxMemory()
	// Type of instance running
	if s.config.followHost() == "" {
		m["propgeo_type"] = "leader"
	} else {
		m["propgeo_type"] = "follower"
	}
	// Whether or not the server is read-only
	m["propgeo_read_only"] = s.config.readOnly()
	// Size of pointer
	m["propgeo_pointer_size"] = (32 << uintptr(uint64(^uintptr(0))>>63)) / 8
	// Uptime of the PropGeo server in seconds
	m["propgeo_uptime_in_seconds"] = time.Since(s.started).Seconds()
	// Number of currently connected PropGeo clients
	s.connsmu.RLock()
	m["propgeo_connected_clients"] = len(s.conns)
	s.connsmu.RUnlock()
	// Whether or not a cluster is enabled
	m["propgeo_cluster_enabled"] = false
	// Whether or not the PropGeo AOF is enabled
	m["propgeo_aof_enabled"] = s.opts.AppendOnly
	// Whether or not an AOF shrink is currently in progress
	m["propgeo_aof_rewrite_in_progress"] = s.shrinking
	// Length of time the last AOF shrink took
	m["propgeo_aof_last_rewrite_time_sec"] = s.lastShrinkDuration.Load() / int64(time.Second)
	// Duration of the on-going AOF rewrite operation if any
	var currentShrinkStart time.Time
	if currentShrinkStart.IsZero() {
		m["propgeo_aof_current_rewrite_time_sec"] = 0
	} else {
		m["propgeo_aof_current_rewrite_time_sec"] = time.Since(currentShrinkStart).Seconds()
	}
	// Total size of the AOF in bytes
	m["propgeo_aof_size"] = s.aofsz
	// Whether or no the HTTP transport is being served
	m["propgeo_http_transport"] = s.http
	// Number of connections accepted by the server
	m["propgeo_total_connections_received"] = s.statsTotalConns.Load()
	// Number of commands processed by the server
	m["propgeo_total_commands_processed"] = s.statsTotalCommands.Load()
	// Number of webhook messages sent by server
	m["propgeo_total_messages_sent"] = s.statsTotalMsgsSent.Load()
	// Number of key expiration events
	m["propgeo_expired_keys"] = s.statsExpired.Load()
	// Number of connected slaves
	m["propgeo_connected_slaves"] = len(s.aofconnM)
	...
	m["propgeo_num_points"] = points
	// Number of objects in the database
	m["propgeo_num_objects"] = objects
	// Number of string in the database
	m["propgeo_num_strings"] = strings
	// Number of collections in the database
	m["propgeo_num_collections"] = s.cols.Len()
	// Number of hooks in the database
	m["propgeo_num_hooks"] = s.hooks.Len()
	// Number of hook groups in the database
	m["propgeo_num_hook_groups"] = s.groupHooks.Len()
	// Number of object groups in the database
	m["propgeo_num_object_groups"] = s.groupObjects.Len()
