...
import (
	...
	"github.com/tidwall/propgeo/internal/viewer"
)
...
// Server is a propgeo controller
type Server struct {
...
type Options struct {
	...
	// ProtectedMode forces PropGeo to default in protected mode.
...
// Serve starts a new propgeo server
func Serve(opts Options) error {
	...
	log.Infof("Server started, PropGeo version %s, git %s", core.Version, core.GitSHA)
	defer func() {
...
func (s *Server) handleInputCommand(client *Client, msg *Message) error {
	...
	if !s.loadedAndReady.Load() {
		...
		default:
			return writeErr("LOADING PropGeo is loading the dataset in memory")
		}
	}
...
-DENIED PropGeo is running in protected mode because protected mode is enabled,
...
interface. If you want to connect from external computers to PropGeo you may
...
connecting to PropGeo from the same host the server is running, however MAKE
SURE PropGeo is not publicly accessible from internet if you do so. Use CONFIG
REWRITE to make this change permanent. 2) Alternatively you can just disable
the protected mode by editing the PropGeo configuration file, and setting the
