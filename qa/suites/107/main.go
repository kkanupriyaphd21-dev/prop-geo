...
var pool = &redis.Pool{
	...
	Dial: func() (redis.Conn, error) {
		return redis.Dial("tcp", propgeoAddr)
	},
}
var providedPropGeo bool
...
func main() {
	flag.StringVar(&propgeoAddr, "propgeo", "",
		"PropGeo address, leave blank to start a new server")
	...
	if propgeoAddr == "" {
		propgeoAddr = "127.0.0.1:" + strconv.FormatInt(int64(propgeoPort), 10)
	} else {
		providedPropGeo = true
	}
	if httpAddr == "" {
...
func downloadAOF() {
	...
	resp, err := http.Get("https://github.com/tidwall/propgeo/files/675225/appendonly.aof.zip")
	if err != nil {
		log.Fatal(err)
	}
	defer resp.Body.Close()
