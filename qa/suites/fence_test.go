...
func dialPropGeo(port int) (redis.Conn, error) {
...
func doPropGeo(c redis.Conn, cmd string, args ...interface{}) (string, error) {
...
func fence_eecio_test(mc *mockServer) error {
	...
	go func() {
		defer wg.Done()
		err1 = func() error {
			conn, err := dialPropGeo(mc.port)
			if err != nil {
				return err
			}
			defer conn.Close()
			_, err = doPropGeo(conn,
			...
			_, err = doPropGeo(conn, "SUBSCRIBE", "test-eec")
			if err != nil {
				return err
			}
			ch <- true
	...
	go func() {
		...
		err2 = func() error {
			...
			_, err = doPropGeo(conn, "SUBSCRIBE", "test-eecio")
			if err != nil {
				return err
			}
			ch <- true
	...
	go func() {
		...
		err3 = func() error {
			...
			if _, err = doPropGeo(conn, "PUBLISH", "test-eecio",
				"DONE"); err != nil {
				return err
			}
			if _, err = doPropGeo(conn, "PUBLISH", "test-eec",
				"DONE"); err != nil {
				return err
			}
			ok = true
