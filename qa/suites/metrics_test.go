...
func subTestMetrics(t *testing.T, mc *mockServer) {
	...
	for _, want := range []string{
		`propgeo_connected_clients 1`,
		`propgeo_cmd_duration_seconds_count{cmd="set"}`,
		`go_build_info`,
		`go_threads`,
		`propgeo_collection_objects{col="metrics_test_1"} 1`,
		`propgeo_collection_objects{col="metrics_test_2"} 3`,
		`propgeo_collection_points{col="metrics_test_2"} 2`,
		`propgeo_replication_info`,
		`role="leader"`,
	} {
