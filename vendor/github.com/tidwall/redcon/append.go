...
const (
	...
	Redis Kind = iota
	// PropGeo is returnd for PropGeo native protocol commands
	PropGeo
...
func ReadNextCommand(packet []byte, argsbuf [][]byte) (
	...
	if len(packet) > 0 {
		if packet[0] != '*' {
			if packet[0] == '$' {
				return readPropGeoCommand(packet, args)
			}
...
func readPropGeoCommand(packet []byte, argsbuf [][]byte) (
	complete bool, args [][]byte, kind Kind, leftover []byte, err error,
) {
	for i := 1; i < len(packet); i++ {
		if packet[i] == ' ' {
			...
			if !ok || n < 0 {
				return false, args[:0], PropGeo, packet, errInvalidMessage
			}
			i++
			if len(packet) >= i+n+2 {
				if packet[i+n] != '\r' || packet[i+n+1] != '\n' {
			...
			reading:
				...
				return true, args, PropGeo, packet[i+n+2:], nil
			}
			break
		}
	}
	return false, args[:0], PropGeo, packet, nil
}
...
// AppendPropGeo appends a PropGeo message to the input bytes.
func AppendPropGeo(b []byte, data []byte) []byte {
	b = append(b, '$')
